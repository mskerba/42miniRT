...
/*   Updated: 2022/11/05 15:28:28 by momeaizi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tuple	cyl_normal_at(t_object *obj, t_tuple *w_point)
{
	...
	if (dist < pow(obj->radius, 2.0) && o_point.y >= obj->cyl_max - EPSILON)
		o_normal = create_tuple(0.0, 1.0, 0.0, 0);
	else if (dist < pow(obj->radius, 2.0) && o_point.y <= obj->cyl_min + EPSILON)
		o_normal = create_tuple(0.0, -1.0, 0.0, 0);
	else
